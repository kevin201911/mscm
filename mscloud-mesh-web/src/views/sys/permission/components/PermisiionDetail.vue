  ...
  <el-card class="form-container" shadow="never">
    <el-form :model="permission" :rules="rules" ref="permissionFrom" label-width="150px">
      ...
      <el-form-item label="父级菜单">
        <el-select v-model="permission.pid" placeholder="请选择">
          <el-option
...
<script>
  import {createPermission, getPermission, updatePermission,findAlls} from '@/api/sys/permission'
    ...
    created() {
      if (this.isEdit) {
        getPermission(this.$route.query.id).then(response => {
          ...
          consloe.log(this.permission)
        });
      }else{
        this.permission = Object.assign({},defaultPermission);
        findAlls().then(response=>{
